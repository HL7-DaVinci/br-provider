...
4. **H2 in-memory database** - Data is transient and reloaded each startup
5. **Do not edit `routeTree.gen.ts`** - Auto-generated by TanStack Router plugin from `src/routes/`
6. **Use `bun`** instead of `npm`, `npx`, or `node` for all frontend/workspace commands
7. **Keep package scripts runtime-neutral** - script bodies invoke `node`, never `bun`, so developers without bun can run them. `bun.lock` stays the only checked-in lockfile.

## Build & Run Commands

This is an Nx workspace. Prefer running tasks through `nx` instead of underlying tooling.

`bun` is the recommended toolchain and the commands below use it. The package
scripts themselves name `node` internally, so `npm run <script>` works for
developers who cannot install bun. Playwright and the E2E stack harness run
under whichever runtime starts them.
...
# Install the Playwright browser once per machine. Installing dependencies does
# not download it. `e2e:stack` runs this step for you.
cd frontend && bunx playwright install chromium

# Where the browser CDN is unreachable, run an installed system browser instead.
# Accepts chrome or msedge. Edge ships with Windows. This skips the download.
E2E_BROWSER_CHANNEL=msedge bun e2e:stack

# Run Playwright E2E tests (requires the full dev stack running; see frontend/playwright.config.ts)
cd frontend && bun e2e

# Run Playwright E2E tests against a self-contained Docker stack (boots and tears down
# the repo root docker-compose.yml). Needs Docker plus bun or npm. The harness is
# plain JS (e2e/run-stack.mjs), so it works on Windows without a bash shell.
cd frontend && bun e2e:stack   # or: npm run e2e:stack
...
- **Testing**: Vitest + Testing Library (unit, `src/**/*.test.*`); Playwright (E2E, `e2e/`, run with `bun e2e` against the live dev stack, or `bun e2e:stack` to boot a self-contained Docker stack). A preflight (`e2e/global-setup.ts`) verifies each stack service is reachable before tests run. Candle-dependent tests get Patient1, its Coverages, Practitioner1 and Payer2 from the CARIN Blue Button examples, loaded by fhir-candle via `-p hl7.fhir.us.carin-bb#2.1.0 --load-examples`. Only bespoke instances are checked in under `e2e/fixtures/seed/`. Package examples win id collisions, so a checked-in file cannot override one.
- **E2E maintenance**: The Playwright tests in `frontend/e2e/` drive the UI by visible text and accessible names (button labels, error messages, sheet titles, status badges). When you change UI labels, workflow steps, or CDS/PAS response handling, update the affected specs in the same change and re-run `bun e2e` with the stack up to confirm they pass.

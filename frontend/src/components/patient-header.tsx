...
import { useOrganization } from "@/hooks/use-clinical-api";
...
export function PatientHeader({ patient, stats }: PatientHeaderProps) {
  ...
  const { data: coverageBundle, isLoading: coverageLoading } = useCoverage(patientId);
  const coverage = coverageBundle?.entry?.[0]?.resource as any;
  const orgRef = coverage && Array.isArray(coverage?.payor) && coverage.payor[0]?.reference
  ...
  return (
    <div className="p-4 bg-card border-b">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-muted-foreground">Today's Date : {todayStr}</span>
      </div>
